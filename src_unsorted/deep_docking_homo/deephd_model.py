...
class DilatedResNetSEBlock(nn.Module):

    def __init__(self, inpc: int, outc: int, dilation: U[int, tuple, list] = 1):
        super().__init__()
        self.inpc = inpc
        self.outc = outc
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return x, aux_i[:, :9], aux_j[:, :9], aux_i[:, 9:9 + 37], aux_j[:, 9:9 + 37], aux_i[:,
                                                                                      9 + 37:9 + 2 * 37], aux_j[:,
                                                                                                          9 + 37:9 + 2 * 37]
